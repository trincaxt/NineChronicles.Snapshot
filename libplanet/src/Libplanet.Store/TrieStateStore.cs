...
namespace Libplanet.Store
{
    ...
    public partial class TrieStateStore : IStateStore
    {
            ...
            int parallelism = 0)
        {
            ...
            long count = 0;
            _logger.Information(
                "CopyStates starting with {Threads} threads, batch size {BatchSize}",
                threads, batchSize);
            stopwatch.Start();
            ...
            foreach (HashDigest<SHA256> stateRootHash in stateRootHashes)
            {
                ...
                // Phase 1: Copy the top-level trie KV pairs using batch writes.
                var batch = new Dictionary<KeyBytes, byte[]>(batchSize);
                foreach (var (key, value) in stateTrie.IterateKeyValuePairs())
                {
                    batch[key] = value;
                    if (batch.Count >= batchSize)
                    {
                        targetKeyValueStore.Set(batch);
                        count += batch.Count;
                        batch.Clear();
                    }
                }
                ...
                _logger.Information(
                    "Phase 1 complete: copied {Count} top-level KV pairs in {Elapsed} ms",
                ...
                if (stateTrie.Get(new KeyBytes(Array.Empty<byte>())) is { } metadata)
                {
                    ...
                    _logger.Information(
                        "Phase 2: processing {AccountStateCount} account states in parallel",
                        accountStateData.Count);
                    ...
                    Parallel.ForEach(
                        ...
                        () => new Dictionary<KeyBytes, byte[]>(batchSize),
                        (accountStateItem, loopState, localBatch) =>
                        {
                            ...
                            {
                                localBatch[key] = value;
                                if (localBatch.Count >= batchSize)
                                {
                                    targetKeyValueStore.Set(localBatch);
                            ...
                            System.Threading.Interlocked.Add(ref parallelCount, localCount);
                            return localBatch;
                        },
                        _ => { });
                    ...
                    _logger.Information(
                        "Phase 2 complete: copied {ParallelCount} account KV pairs in {Elapsed} ms",
                        parallelCount, stopwatch.ElapsedMilliseconds);
                }
            }
